...
\begin{abstract}
Leadership communication is widely assumed to move financial markets, and a growing literature reports directional accuracies of 74--87\% from NLP pipelines built on such text. We construct one of these pipelines end to end and test the assumption directly. Our corpus comprises 1{,}288 communications spanning May 2016 to August 2026 --- 746 U.S. Federal Reserve and 472 European Central Bank documents alongside 70 Mann ki Baat broadcasts, so 94.6\% of the corpus is central-bank rather than domestic political text --- aligned to daily prices for 16 Indian and global instruments, yielding 16{,}832 event observations. We evaluate under a cross-sectional design whose baseline is pinned at exactly 50\% by construction, with expanding-window walk-forward refits and standard errors clustered by event date. Rhetoric features score 49.99\% ($z=-0.63$), and a horizon sweep from 1 to 252 trading days confines rhetoric-only accuracy to 49.88--50.02\% at every lag, ruling out delayed transmission. Under the absolute-direction framing, fitted models underperform a naive always-up rule by 5.86 percentage points ($z=-6.00$). Granger tests across ten topics yield zero discoveries surviving Benjamini--Hochberg correction, against a procedure whose null expectation is 2.26 nominal hits. As a positive control the same pipeline predicts realized-volatility direction at 68.67\% ($+17.07$pp over baseline, $z=+23.75$), establishing that the harness detects structure where structure exists; rhetoric contributes nothing there either ($-0.62$pp, $p=0.17$). Two design bounds qualify the null: the corpus composition above, and FinBERT's 512-token limit, which exposes only 10.7\% of a median transcript to the sentiment model. We therefore claim no \emph{detectable} directional signal under this design rather than a general absence. Finally we quantify how evaluation design manufactures accuracy: tuning the horizon to 252 days reports 75.28\%, a headline-grade figure that is 3.58pp \emph{worse} than a constant rule on the same rows. We release the pipeline and evaluation harness.
\end{abstract}
