...
To establish that the null reflects the data rather than a weak pipeline, we include a positive control: the same features and protocol predict realized-volatility direction at 68.67\%, $+17.07$ percentage points over baseline at $z=+23.75$. The machinery detects structure when structure is present. What it does not detect is any directional signal in the rhetoric features.

We state the scope of that claim precisely, because two properties of this design bound it. First, the corpus is 94.6\% central-bank communication: 1{,}218 ECB and Federal Reserve documents against 70 Mann ki Baat broadcasts. What we test is therefore predominantly institutional monetary communication, not domestic political rhetoric, and the Indian political channel is measured on thin data. Second, FinBERT's 512-token limit means our sentiment features observe only 10.7\% of a median Mann ki Baat transcript. The correct reading of our result is \textit{no detectable directional signal under this design} --- daily resolution, truncated document-level sentiment, and a corpus dominated by central banks --- rather than a general claim that leadership communication is uninformative.
...
\textbf{Multiple testing and overfitting.} White \cite{white2000} introduced the reality-check bootstrap for data snooping. Harvey et al.\ \cite{harvey2016} argue most published cross-sectional predictors are false discoveries under proper multiplicity accounting. Bailey et al.\ \cite{bailey2014} formalize backtest overfitting. Benjamini and Hochberg \cite{benjamini1995} supply the FDR procedure we apply. Gu et al.\ \cite{gu2020} provide the evaluation template we follow.

\textbf{Synthesis: two literatures that do not cite each other.} Read
together, the studies above fall into two groups that reach incompatible
conclusions and largely ignore one another. The applied text-and-markets
work \cite{bhandari2025, jain2023, biswal2022, seng2023, rambola2021}
reports directional accuracies of 74--87\% and $R^2$ up to 0.9866, while the
methodological work in asset pricing \cite{white2000, harvey2016,
bailey2014, gu2020} holds that effects of that size almost never survive
multiplicity correction and honest out-of-sample protocol. Both cannot be
right about the same markets.

Three specific gaps explain the divergence, and each maps to a design choice
here. First, \textit{baselines are usually unreported}: none of the applied
studies we reviewed states what a constant or persistence rule achieves on
its own test window, which is precisely the comparison that separates
...
significant topic and zero after FDR adjustment, an inversion that an
uncorrected reading would report as a finding. Nazari et al.\
\cite{nazari2025} is the closest existing warning from inside the applied
literature; we supply the quantitative version of it, on data where the
answer is known because we also measured what a working signal looks like.
